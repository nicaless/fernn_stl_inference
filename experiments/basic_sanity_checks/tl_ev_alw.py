...
from TLOps import TLAlw, TLEv, TLAtom
...
ev_output = RNN(TLEv(trace_length), return_sequences=True)(alw_output)
...
np.savetxt(DATA_INPUT_FOLDER + 'rnn_predictions_ev_alw.csv',
...
with open(DATA_INPUT_FOLDER + 'training_results.txt', 'a') as f:
    if np.array_equal(original_weights, weights):
       result = 'Fail: TL Ev(Alw) - Weights Adjusted After Training'
    else:
       result = 'Pass: TL Ev(Alw) - Weights Adjusted After Training'
