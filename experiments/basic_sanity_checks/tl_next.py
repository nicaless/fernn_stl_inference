...
from TLOps import TLNext, TLAtom
...
chi = RNN(TLAtom(1, w=1, b=0), return_sequences=True)(in1)
ev_output = RNN(TLNext(trace_length), return_sequences=True)(chi)
model = Model(inputs=[in1], outputs=[ev_output])
model.compile(optimizer='adam',
              loss='binary_crossentropy',
              metrics=['accuracy'])

predictions = model.predict([data3], verbose=0)
predictions_save = np.reshape(predictions, (100, 5))

np.savetxt(DATA_INPUT_FOLDER + 'rnn_predictions_next.csv',
           predictions_save, delimiter=',', fmt='%.5e')

# MAKE TRAINABLE TL EV
in1 = Input(input_shape)
phi = RNN(TLAtom(1), return_sequences=True)(in1)
ev_output = RNN(TLNext(trace_length), return_sequences=True)(phi)
trainable_model = Model(inputs=[in1], outputs=[ev_output])
...
with open(DATA_INPUT_FOLDER + 'training_results.txt', 'a') as f:
    if np.array_equal(original_weights, weights):
       result = 'Fail: TL Next - Weights Adjusted After Training'
    else:
       result = 'Pass: TL Next - Weights Adjusted After Training'
