...
from TLOps import TLUntil, TLAtom
...
until_output = RNN(TLUntil(trace_length), return_sequences=True)(until_input)
...
np.savetxt(DATA_INPUT_FOLDER + 'rnn_predictions_until.csv',
...
until_input = concatenate([phi, psi])
until_output = RNN(TLUntil(trace_length), return_sequences=True)(until_input)
trainable_model = Model(inputs=[in1, in2], outputs=[until_output])
...
with open(DATA_INPUT_FOLDER + 'training_results.txt', 'a') as f:
    if np.array_equal(original_weights, weights):
       result = 'Fail: TL Until - Weights Adjusted After Training'
    else:
       result = 'Pass: TL Until - Weights Adjusted After Training'
