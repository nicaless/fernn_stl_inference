...
ev_output = RNN(TLEv(trace_length), return_sequences=True)(phi)
...
np.savetxt(DATA_INPUT_FOLDER + 'rnn_predictions_ev.csv',
...
with open(DATA_INPUT_FOLDER + 'training_results.txt', 'a') as f:
    if np.array_equal(original_weights, weights):
       result = 'Fail: TL Ev - Weights Adjusted After Training'
    else:
       result = 'Pass: TL Ev - Weights Adjusted After Training'
