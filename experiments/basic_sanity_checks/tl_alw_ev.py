...
ev_output = RNN(TLAlw(trace_length), return_sequences=True)(phi)
alw_output = RNN(TLEv(trace_length), return_sequences=True)(ev_output)
...
np.savetxt(DATA_INPUT_FOLDER + 'rnn_predictions_alw_ev.csv',
...
with open(DATA_INPUT_FOLDER + 'training_results.txt', 'a') as f:
    if np.array_equal(original_weights, weights):
       result = 'Fail: TL Alw(Ev) - Weights Adjusted After Training'
    else:
       result = 'Pass: TL Alw(Ev) - Weights Adjusted After Training'
