...
from TLOps import TLAtom, TLNext, TLUntil
...
CHOICE_CONFIG = {'choice_unt_nxt': ['unt', 'nxt']}
...
until = RNN(TLUntil(trace_length), return_sequences=True, name='unt')(until_input)

nxt = RNN(TLNext(trace_length), return_sequences=True, name='nxt')(chi)

choice = concatenate([until, nxt])
choose_nxt = TLWeightTransform(1, w=[0, 1], transform='sum', name='choice_unt_nxt')(choice)  # PICK NEXT

model = Model(inputs=[in1, in2, in3], outputs=[choose_nxt])
...
np.savetxt(DATA_INPUT_FOLDER + 'rnn_predictions_select_nxt.csv',
...
choice = concatenate([until, nxt])
choose_nxt = TLWeightTransform(1, quantize='one-hot', transform='sum', name='choice_unt_nxt')(choice)

trainable_model = Model(inputs=[in1, in2, in3], outputs=[choose_nxt])
...
with open(DATA_INPUT_FOLDER + 'training_results.txt', 'a') as f:
    if np.array_equal(original_weights, weights):
       result = 'Fail: TL Select Unt or Nxt - Weights Adjusted After Training'
    else:
       result = 'Pass: TL Select Unt or Nxt - Weights Adjusted After Training'
